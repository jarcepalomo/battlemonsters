...
export function ComicBattleInterface() {
  ...
  // Function to group panels into rows of 3 with predefined layouts and skeletons
  const groupPanelsIntoRows = (panels: BattlePanel[]) => {
    const rows = [];
    const allPanels = [...panels];
    
    for (let i = 0; i < allPanels.length; i += 3) {
      const rowPanels = allPanels.slice(i, i + 3);
      const rowIndex = Math.floor(i / 3);
      const layoutPattern = getLayoutPattern(rowIndex);
      
      // If this is the first panel of a new row (not complete), add skeleton placeholders
      if (rowPanels.length < 3) {
        const skeletonsNeeded = 3 - rowPanels.length;
        for (let j = 0; j < skeletonsNeeded; j++) {
          rowPanels.push({
            id: `skeleton-${i + rowPanels.length + j}`,
            isGenerating: false,
            error: false,
            description: '',
            prompt: '',
              ...
              {battlePanels.length === 0 ? (
                ...
                <div className="space-y-6">
                  {panelRows.map((row, rowIndex) => (
                    <div key={rowIndex} className="flex gap-4 h-64">
